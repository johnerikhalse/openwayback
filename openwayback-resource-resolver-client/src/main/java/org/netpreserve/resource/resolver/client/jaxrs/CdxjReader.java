 ...
 */
@Provider
@Consumes("application/vnd.org.netpreserve.cdxj")
public class CdxjReader implements MessageBodyReader<List<CdxjLine>> {
    ...
    @Override
    public List<CdxjLine> readFrom(Class<List<CdxjLine>> type, Type genericType, Annotation[] annotations, MediaType mediaType,
            MultivaluedMap<String, String> httpHeaders, InputStream entityStream)
            throws IOException, WebApplicationException {

        List<CdxjLine> result = new ArrayList<>();
        try (BufferedReader in = new BufferedReader(new InputStreamReader(entityStream))) {
            String line;
            while ((line = in.readLine()) != null) {
                result.add((CdxjLine) CdxRecordFactory.create(line, CdxjLineFormat.DEFAULT_CDXJLINE));
            }
        }
